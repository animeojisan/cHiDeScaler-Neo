...
impl GraphicsCaptureApi {
    ...
    >(
        ...
        // Create frame pool
        // A single WGC buffer drops incoming compositions whenever the frame
        // callback is still mapping/copying the previous texture. Three
        // buffers are the Windows capture convention: one may be presented,
        // one processed, and one ready for the next DWM composition.
        let frame_pool =
        ...
        >::new({
            ...
            move |frame, _| {
                ...
                let event_pool = frame
                    .as_ref()
                    .expect("FrameArrived parameter was None this should never happen.");
                // FrameArrived notifications may coalesce while the previous
                // callback is copying a large texture. Drain every frame that
                // is already buffered instead of consuming only one and
                // leaving the pool permanently one composition behind.
                for _ in 0..3 {
                    let Ok(frame) = event_pool.TryGetNextFrame() else {
                        break;
                    };
