...
impl<'a> DxgiDuplicationFrame<'a> {
    ...
    pub fn buffer<'b>(&'b mut self) -> Result<DxgiDuplicationFrameBuffer<'b>, Error> {
        ...
        unsafe {
            self.d3d_device
                .CreateTexture2D(&texture_desc, None, Some(&mut staging))?;
        };
        ...
        unsafe {
            self.d3d_device_context
                .CopyResource(&staging, &self.texture);
        };
        ...
        let mapped_frame_data = unsafe {
            slice::from_raw_parts_mut(
                mapped.pData.cast(),
                (self.texture_desc.Height * mapped.RowPitch) as usize,
            )
        };
    ...
    pub fn buffer_crop<'b>(
        ...
        let mapped_frame_data = unsafe {
            slice::from_raw_parts_mut(
                mapped.pData.cast(),
                (texture_height * mapped.RowPitch) as usize,
            )
        };
    ...
    pub fn buffer_crop_with<'s>(
        ...
        let mapped_frame_data = unsafe {
            slice::from_raw_parts_mut(
                mapped.pData.cast(),
                (crop_height * mapped.RowPitch) as usize,
            )
        };
