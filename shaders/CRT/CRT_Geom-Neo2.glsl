...
//!PARAM NeoToneGamma
//!DESC Neo Tone Preservation
//!TYPE CONSTANT float
//!MINIMUM 1.0
//!MAXIMUM 2.4
1.00

//!PARAM NeoToneLiftGamma
//!DESC Neo Dark/Midtone Lift Gamma
//!TYPE CONSTANT float
//!MINIMUM 0.40
//!MAXIMUM 1.00
0.76

//!PARAM NeoToneLiftStrength
//!DESC Neo Dark/Midtone Lift Strength
//!TYPE CONSTANT float
//!MINIMUM 0.0
//!MAXIMUM 1.0
0.54

//!PARAM NeoColorDensity
//!DESC Neo CRT Color Density
//!TYPE CONSTANT float
//!MINIMUM 1.0
//!MAXIMUM 1.5
1.24

//!PARAM NeoLumaContrast
//!DESC Neo CRT Luma Contrast
//!TYPE CONSTANT float
//!MINIMUM 1.0
//!MAXIMUM 1.2
1.05
...
vec4 hook()
{
    ...
    // Keep the original CRT input/output gamma model, but soften only the
    // second transfer used by this Neo2 variant so dark colors retain
    // separation instead of collapsing toward black.
    col = pow(col, vec4(NeoToneGamma));
    col2 = pow(col2, vec4(NeoToneGamma));
    ...
    // Neo2: lift luminance rather than each RGB channel independently.
    // This keeps the original CRT color ratios much more intact, avoiding
    // the washed-out look caused by per-channel shadow lifting.
    vec3 baseTone = clamp(mul_res, 0.0, 1.0);
    const vec3 kNeoLuma = vec3(0.2126, 0.7152, 0.0722);
    float baseLuma = dot(baseTone, kNeoLuma);
    float liftedLuma = mix(
        baseLuma,
        pow(max(baseLuma, 0.0), NeoToneLiftGamma),
        NeoToneLiftStrength
    );

    float lumaGain = (baseLuma > 1e-6) ? (liftedLuma / baseLuma) : 1.0;
    vec3 liftedTone = clamp(baseTone * lumaGain, 0.0, 1.0);

    // Restore a small amount of CRT-style color density without changing
    // neutral greys. The effect naturally diminishes near clipping.
    float outLuma = dot(liftedTone, kNeoLuma);

    // Give Neo2 a little more of the dense CRT-Geom character without
    // crushing near-black detail. Contrast is applied around a low pivot,
    // then chroma density is restored independently.
    float pivot = 0.30;
    float denseLuma = clamp((outLuma - pivot) * NeoLumaContrast + pivot, 0.0, 1.0);

    vec3 chroma = liftedTone - vec3(outLuma);
    vec3 denseTone = vec3(denseLuma) + chroma * NeoColorDensity;
    mul_res = clamp(denseTone, 0.0, 1.0);
