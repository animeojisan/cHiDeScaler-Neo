/*
    Mitchell-Netravali Resize Adjustable for cHiDeScaler-Neo v1.0
    ------------------------------------
...
//!HEIGHT HOOKED.h
//!DESC Mitchell-Netravali Resize Adjustable Horizontal v1.0
vec4 hook()
{
    ...
    for (int tap = 0; tap < 64; ++tap)
    {
        ...
        // Mitchell-Netravali cubic, B = 1/3 and C = 1/3.
        if (x < 1.0)
        {
            weight = ((7.0 * x - 12.0) * x * x + 16.0 / 3.0) / 6.0;
        }
        else if (x < 2.0)
        {
            weight = ((-7.0 / 3.0 * x + 12.0) * x - 20.0) * x / 6.0
                   + 16.0 / 9.0;
        }
...
//!DESC Mitchell-Netravali Resize Adjustable Vertical v1.0
vec4 hook()
{
