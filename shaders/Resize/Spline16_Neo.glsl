/*
    Spline16 Resize Adjustable for cHiDeScaler-Neo v1.0
    --------------------------
...
//!HEIGHT HOOKED.h
//!DESC Spline16 Resize Adjustable Horizontal v1.0
vec4 hook()
{
    ...
    for (int tap = 0; tap < 64; ++tap)
    {
        ...
        if (x < 1.0)
        {
            weight = ((x - 9.0 / 5.0) * x - 1.0 / 5.0) * x + 1.0;
        }
        else if (x < 2.0)
        {
            x -= 1.0;
            weight = ((-1.0 / 3.0 * x + 4.0 / 5.0) * x
                    - 7.0 / 15.0) * x;
        }
...
//!DESC Spline16 Resize Adjustable Vertical v1.0
vec4 hook()
{
